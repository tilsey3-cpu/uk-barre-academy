...
- There is no in-page registration form and no Typeform — booking is direct Stripe checkout. The two course dates, their times, and their Stripe Payment Link URLs live in `app/src/dates.js` (`DATE_GROUPS`), the single source of truth reused by `Dates.jsx`, `Fee.jsx`, and `DateModal.jsx`. The Fee card's "Book your date" buttons and the Dates section's per-card "Book this date" buttons link straight to the matching Stripe URL in a new tab. Every generic "Become a Founding Instructor" CTA (header, mobile menu, hero, closing CTA section) instead opens `DateModal.jsx` — a Radix `Dialog` (state lifted to `App.jsx`, opened via an `onOpenDateModal` prop) showing both dates as buttons that link to their Stripe URLs.
